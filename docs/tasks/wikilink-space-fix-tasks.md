...
- **テスト要件**:
  ...
  - [ ] 単体テスト: `[[  page  |  Display Name  ]]` → エイリアス空白保持
  - [ ] 境界値テスト: タブ文字、改行文字の処理
- **完了条件**:
  - [ ] 先頭・末尾空白が自動除去される
  - [ ] エイリアス部分の空白は保持される
...
- **実装詳細**:
  ...
  - 日本語文字を含む見出しの適切な処理
- **テスト要件**:
  - [ ] 単体テスト: `[[page#Test Heading]]` → `/blog/page#test-heading`
  - [ ] 単体テスト: `[[page#日本語 見出し]]` → `/blog/page#日本語-見出し`
  - [ ] 統合テスト: エイリアス付き見出しリンク
- **完了条件**:
  - [ ] 見出しアンカーのスペースが適切に処理される
  - [ ] 日本語と英語の混在見出しが正しく動作する
...
- **実装詳細**:
  ...
  - パイプ文字の一時的置換処理の拡張
- **テスト要件**:
  - [ ] 単体テスト: テーブル内wikilinkの正常動作
  - [ ] 統合テスト: 複雑なテーブル構造でのwikilink
  - [ ] 回帰テスト: 既存のテーブルテストが通過する
- **エラーハンドリング**:
  - [ ] ネストしたテーブル構造での処理
  - [ ] 不正なパイプ文字の処理
- **完了条件**:
  - [ ] テーブル内でwikilinkが正常に動作する
  - [ ] パイプ文字の競合が発生しない
